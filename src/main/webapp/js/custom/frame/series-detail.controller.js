(function () {
    ...
    SeriesDetailController.$inject = ['$scope', 'CoverService', 'SweetAlert', 'NgTableParams', 'ngDialog', 'blockUI', 'Notify'];

    function SeriesDetailController($scope, CoverService, SweetAlert, NgTableParams, ngDialog, blockUI, Notify) {
        ...
        $scope.publish = function () {
            //todo判断是否都填写完整了?
            var data = $scope.obj;
            CoverService.publishSeriesDetail(data).success(function (data) {
                if (data.status == 200) {
